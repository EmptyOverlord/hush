...
- **Ускорить саму речь** до ×2, пока паузы вырезаются
...
- **Сглаживание** склеивает микро-резы, чтобы речь не дёргалась
...
## Субтитры, без интернета

Hush умеет и распознавать речь. Всё считается **на твоей машине** — ничего
никуда не уходит, и после скачивания модели интернет не нужен.

- **SRT** — сразу заливать на YouTube, или просто текст
- **По одному слову в строке** — те самые прыгающие субтитры для Shorts
- **Перевод на английский** прямо при распознавании: записал по-русски,
  получил английские субтитры за один проход
- Восемь моделей на выбор, от `tiny` (74 МБ) до `large-v3` (2.9 ГБ).
  Квантованные дают большую модель в разы меньшим весом — **turbo-q5
  (547 МБ)** золотая середина: качество large-v3 при весе `small`.
  Для русского начинай с неё.

Модель качается один раз и остаётся в твоей пользовательской папке.
